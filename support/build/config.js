...
module.exports = function(app, cwd) {
  ...
  app.store = new Store({path: paths.data('generate.json')});
  ...
  // app.data('site.lead', 'Getting started with Generate.');
  app.data('site.org.url', 'https://github.com/generate');
  app.data('site.nav.main', ['docs', 'plugins', 'masthead']);
  ...
  app.helpers(helpers());
  app.helper('geopattern', geopattern({color: '#55c1a5'}));
  app.helper('geoColor', geopattern.color({color: '#55c1a5'}));
  ...
  // return build paths to use in assemblefile.js
  return paths;
};
