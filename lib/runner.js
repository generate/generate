...
module.exports = function(configfile, generatorFn) {
  return function(app, cb) {
    ...
    app.runner(configfile, function(err, options, app) {
      ...
      // if no configfile is in the user's cwd, use the default
      if (!app.hasConfigfile && typeof generatorFn === 'function') {
        app.register('default', generatorFn);
      }

      // set options on the default generator
      var gen = app.getGenerator('default');
      if (gen) {
        gen.option(app.base.options);
        gen.option(options);
      }

      // normalize the config object with `base-config` mappings
      app.config.process(options, function(err) {
        if (err) return cb(err);

        app.cli.process(options, function(err) {
          if (err) return cb(err);
