...
function toTree(options) {
  ...
  return utils.archy(buildTree(cwd));
}
...
function stylize(name, options) {
  ...
  if (name === 'generate') {
    return utils.log.bold(name);
  }

  var match = /(assemble|base|generate|templates)/.exec(name);
  switch (match && match[1]) {
    case 'base':
      name = utils.log.red(name);
      break;
    case 'assemble':
      name = utils.log.cyan(name);
      break;
    case 'generate':
      name = utils.log.green(name);
      break;
    case 'templates':
      name = utils.log.yellow(name);
      break;
    case null:
    default: {
      break;
