...
module.exports = function(argv, app, base, env) {
  if (!argv) {
    throw new Error('expected an argv object');
  }
...
function toTask(task, app, base, env) {
  ...
  // does the current working directory have a configfile?
  var isUserConfig = env.config.cwd && (env.config.cwd === env.user.cwd);
  ...
  if (tasks.length > 1) {
    tasks.forEach(function(t) {
      ...
      var resolved = toTask(t, app, base, env);
      if (resolved === null) {
        var _task = t.split(/\W/).pop();
        var msg = 'task "' + _task + '" is not registered';
        if (generator && generator !== 'base') {
  ...
  if (generator) {
    generator = utils.toGeneratorKey(generator);
  }

  if (generator) {
    var hasGen = utils.has(app, generator);
    if (hasGen) {
      if (utils.has(app, [generator, 'tasks', task])) {
  ...
  var key = utils.toGeneratorKey(task);
  ...
  if (gen && utils.has(gen, ['tasks', task])) {
    return task;
  }

  key = utils.toGeneratorKey(task);
  ...
  var genKey = utils.toGeneratorKey(generator);
  if (utils.has(app, ['env', genKey])) {
    return 'env.' + genKey + ':' + task;
  }
