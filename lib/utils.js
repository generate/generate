...
utils.renameFile = function(app) {
  return function(file, next) {
    ...
    // re-add front-matter string if `keep` is defined
    if (file.data && file.data.keep === true) {
      delete file.data.keep;
      delete file.data.rename;
      delete file.data.ext;
      file.content = utils.matter.stringify(file.content, file.data);
    }
