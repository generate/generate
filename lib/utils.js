...
/**
 * Get the resolved path to an "generatefile.js"
 */

utils.generatefile = function(dir) {
  return path.join(dir, 'generatefile.js');
};
...
utils.matchFiles = function(pattern, options) {
  ...
  while (++i < len) {
    var name = files[i];
    if (name === 'generate-next') continue;
...
utils.register = function(pattern, base, generate, options) {
  utils.matchFiles(pattern, options).forEach(function (fp) {
    ...
    require(utils.generatefile(fp))(app, base);
